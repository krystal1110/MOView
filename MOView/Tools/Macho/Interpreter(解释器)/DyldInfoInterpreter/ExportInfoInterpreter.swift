...
class ExportInfoNode {
    ...
    lazy var transItems: [ExplanationItem] = {
        var transItems: [ExplanationItem] = []
        
        let terminalSizeRange = Utils.makeRange(start: startOffsetInMacho, length: terminalSize.byteCount)
        transItems.append(ExplanationItem(sourceDataRange: startOffsetInMacho..<startOffsetInMacho+terminalSize.byteCount,
                                          model: ExplanationModel(description: "Terminal Size", explanation: "\(terminalSize.rawValue)")))
        
        if let flags = flags,
            let leb128Array = leb128Array {
            
            let flagsRange = Utils.range(after: terminalSizeRange, length: flags.byteCount)
            transItems.append(ExplanationItem(sourceDataRange: flagsRange,
                                              model: ExplanationModel(description: "Kind", explanation: flags.kind.readable)))
            
            transItems.append(ExplanationItem(sourceDataRange: flagsRange,
                                              model: ExplanationModel(description: "Flags", explanation: flags.flagsDescription)))
            ...
            for leb in leb128Array {
                ...
                transItems.append(ExplanationItem(sourceDataRange: thisLEBRange,
                                                  model: ExplanationModel(description: "LEB", explanation: leb.rawValue.hex)))
            ...
            if let reExportedSymbolName = reExportedSymbolName {
                ...
                transItems.append(ExplanationItem(sourceDataRange: flagsRange,
                                                  model: ExplanationModel(description: "ReExported Symbol Name",
        ...
        transItems.append(ExplanationItem(sourceDataRange: edgeCountRnage,
                                          model: ExplanationModel(description: "Number of Edges", explanation: "\(edgesCount.rawValue)")))
        ...
        for index in edges.indices {
            ...
            transItems.append(ExplanationItem(sourceDataRange: edgeNameRange,
                                              model: ExplanationModel(description: "Edge Name",
                                                                              explanation: edge.edgeName.rawValue)))
            
            let edgeOffsetRange = Utils.range(after: edgeNameRange, length: edge.offset.byteCount)
            transItems.append(ExplanationItem(sourceDataRange: edgeOffsetRange,
                                              model: ExplanationModel(description: "Edge Offset",
                                                                              explanation: edge.offset.rawValue.hex)))
            rangeOfLastEdge = edgeOffsetRange
        }
        
        transItems.insert(ExplanationItem(sourceDataRange: nil,
                                          model: ExplanationModel(description: "Trie Node Full Name",
...
//
class ExportInfoInterpreter: Interpreter {
    var dataSlice: Data
