...
//            modules.append(module)
        ...
        }else if (section.sectname == DataSection.mod_init_func.rawValue ||
                  section.sectname == DataSection.const.rawValue){
            let interpreter = ASCIIInterpreter(dataSlice: dataSlice, searchProtocol: searchProtocol)
            let list = interpreter.transitionData()
            let module = UnknownModule(with: dataSlice, section: section,list: list)
            modules.append(module)
        }
