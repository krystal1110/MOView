...
//            print("----")
            ...
            let dataStr = withUnsafePointer(to: &self.macho.instructionPtr![Int(index)].op_str) {
              ...
              asmStr = withUnsafePointer(to: &self.macho.instructionPtr![Int(index)].mnemonic) {
                $0.withMemoryRebound(to: UInt8.self, capacity: MemoryLayout.size(ofValue: self.macho.instructionPtr![Int(index)].mnemonic)) {
                    String(cString: $0)
                }
            }
                        ...
                        if (strStr(dataStr, targetStr)){
                            ...
                            // 命中高位
                            high = true
                        }else if (strStr(dataStr, targetLowStr)) {
                            // 命中低12位
                            if high { return true }
                        }
    ...
    func strStr(_ haystack: String, _ needle: String) -> Bool {
        if  sunday(text: Array(haystack), pattern: Array(needle)) == -1 {
            return false
        }else{
            return true
        }
    ...
    func sunday(text: [Character], pattern: [Character]) -> Int {
